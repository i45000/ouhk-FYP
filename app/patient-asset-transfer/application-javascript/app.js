...
exports.connectToNetwork = async function(doctorID) {
  ...
  try {
    ...
    */
    ...
    // the program sets up the gateway connection with the userId stored in the wallet and specifies discovery options.
 ...
 */
exports.getAllDoctorsByHospitalId = async function(networkObj, hospitalId,userID) {
  // Get the User from the identity context
  const users = await networkObj.gateway.getAll(users);
  let caClient;
  const result = [];
  try {
